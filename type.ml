...
type definition_t =
  ...
  | Array of simple_t * int  (* elementtype, dimension *)
  | Name of simple_t
  | Result of simple_t
  | Control
...
(* In the case of references, assignment
   compatibility is mostly determined at runtime. *)
...
and describe_simple = 
  function
  | Bits -> "BITS"
  | Number(Long, Integer)  -> "an INTEGER"
