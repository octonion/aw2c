(* alwstd.ml -- Algol W headers for the Algol W standard library 
...
let scope =
  ...
  let exception_loc = Location.create "<internal>" 0 0 in
  let exception_id = Table.Id.create "exception" in
  let exception_class = Class.create exception_loc exception_id in
...
(* Standard Transfer Procedures *)
    ...
    ; "base16",      Procedure (String 24, [By_value(real)])
    ; "longbase10",  Procedure (String 20, [By_value(long_real)])
    ; "longbase16",  Procedure (String 24, [By_value(long_real)])
...
(* Predeclared variables. *)
    ...
    ; "ovfl",       exception_reference
    ; "unfl",       exception_reference
    ; "divzero",    exception_reference
    ; "intovfl",    exception_reference
