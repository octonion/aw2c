''' scanner_state_machine.py -- generates a state machine scanner for the Algol W READ statement

c.f. Algol W Language Description, section 6.3
...
MESSAGE = """/* GENERATED BY scanner_state_machine.py: DON'T EDIT THIS FILE, EDIT alwio.c.input */
"""
...
#
# See alwio.c.input for the meanings of the Scanner_* actions.
# (Basically, they are filling a buffer with the C versions of 
# the Algol W constants which will be scanned with 'strtol' etc.)
...
def state_machine():
    f = open('alwio.c.input')
    code = f.read()
    head, tail = code.split('<<STATES>>')
    f.close()
    f = open('alwio.c', 'w')
    states = ''
    for state in sorted(transitions.keys()):
        ...
        states += STATE % {'STATE': state, 'CASES': cases}
    f.write(MESSAGE)
    f.write('#line 2 "alwio.c.input"\n')
    f.write(head)
    f.write('#line 1 "make_alwio_scanner.py"\n')
    f.write(states)
    f.write('#line %d "alwio.c.input"\n' % (head.count('\n') + 2))
    f.write(tail)
    f.close()
